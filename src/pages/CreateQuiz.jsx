...
const CreateQuiz = () => {
  ...
  const handleCreate = async (values) => {
    ...
    try {
      ...
      const quizData = {
        ...
        createdBy: auth.userId // Add creator ID if needed
      };
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_API_URL}/api/quiz/create`,
        {
          ...
          createdBy: auth.userId // We'll still send the creator's ID
        },
        {
          headers: { 
  ...
  const handleQuestionSubmit = () => {
    questionForm.validateFields().then(values => {
      ...
      if (values.type !== 'text' && (!values.options || values.options.length < 2)) {
      ...
      const newQuestion = {
        ...
        type: values.type,
        options: values.type === 'text' ? [] : values.options.filter(opt => opt.trim() !== ''),
                            ...
                            >
                              <Space>
                                <Form.Item
                                  ...
                                  noStyle
                                >
                                  <Input placeholder={`Option ${index + 1}`} style={{ width: '300px' }} />
                                </Form.Item>
                                {fields.length > 2 && (
